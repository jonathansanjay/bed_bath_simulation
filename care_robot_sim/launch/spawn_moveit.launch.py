...
def generate_launch_description():
    ...
    robot_description_file = os.path.join(
        get_package_share_directory('care_robot_sim'), 'urdf', 'cobot_gripper_camera.urdf.xacro'
    )
    joint_controllers_file = os.path.join(
        get_package_share_directory('care_robot_sim'), 'config', 'controllers_trajectory.yaml'
    )
    ...
    world_file = os.path.join(
        get_package_share_directory('care_robot_sim'), 'worlds', 'patient_env.world'
    )
    ...
    planning_time_node = Node(
    package='care_robot_sim',
    executable='planning_time_publisher.py',
    output='screen')

    task_status_node = Node(
    package='cobot_ik',
    executable='task_status_wrapper.py',
    output='screen')
    ...
    ld.add_action(planning_time_node)
    ld.add_action(task_status_node)
